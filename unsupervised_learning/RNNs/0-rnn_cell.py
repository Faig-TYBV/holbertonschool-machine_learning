...
class RNNCell:
    def __init__(self, i, h, o):
        ...
        # h_prev shape is (m, h) and x_t shape is (m, i), so concat is (m, h + i)
    ...
    def forward(self, h_prev, x_t):
        ...
        Returns:
            tuple: (h_next, y)
        """
        # Concatenate previous hidden state and input data along the feature axis
        concat_input = np.concatenate((h_prev, x_t), axis=1)
        
        # Calculate next hidden state (using standard tanh activation for RNNs)
        h_next = np.tanh(np.matmul(concat_input, self.Wh) + self.bh)
        
        # Calculate raw output
        z = np.matmul(h_next, self.Wy) + self.by
        
        # Apply softmax activation (using max subtraction for numerical stability)
