...
class Yolo:
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        """
        Annotate and display an image with YOLO detections.

        Draws each bounding box as a blue rectangle, then writes the class
        name and rounded score in red just above the top-left corner.
        Pressing 's' saves the annotated image to ./detections/<file_name>
        and closes the window; any other key closes without saving.

        Args:
            image (np.ndarray): Original BGR image (as loaded by OpenCV).
            boxes (np.ndarray): Shape (n, 4) — (x1, y1, x2, y2) box coords.
            box_classes (np.ndarray): Shape (n,) — class index per box.
            box_scores (np.ndarray): Shape (n,) — confidence score per box.
            file_name (str): Original file path; used as the window title
                and as the saved filename inside ./detections/.
        """
        import cv2
        import os
        ...
        for box, cls_idx, score in zip(boxes, box_classes, box_scores):
            ...
            # Draw bounding box — blue (BGR: 255, 0, 0), thickness 2
            cv2.rectangle(annotated, (x1, y1), (x2, y2), (255, 0, 0), 2)

            # Build label: "ClassName score"
            label = "{} {:.2f}".format(self.class_names[cls_idx], score)

            # Draw label in red (BGR: 0, 0, 255), 5 px above the top-left
            cv2.putText(
                annotated,
                label,
                (x1, y1 - 5),
        ...
        # Display the annotated image; window title = bare filename
        window_title = os.path.basename(file_name)
        cv2.imshow(window_title, annotated)

        key = cv2.waitKey(0) & 0xFF

        if key == ord('s'):
            # Save to ./detections/<file_name>
    ...
    def predict(self, folder_path):
        """
        Run the full YOLO v3 pipeline on every image in folder_path.

        Pipeline per image:
          1. load_images        — read images from disk
          2. preprocess_images  — resize + normalise to model input spec
          3. model.predict      — run inference (batch)
          4. process_outputs    — decode raw predictions to pixel coords
          5. filter_boxes       — remove low-confidence detections
          6. non_max_suppression — remove duplicate detections per class
          7. show_boxes         — annotate and display; optionally save

        Args:
            folder_path (str): Path to the directory holding the images.

        Returns:
            tuple:
                predictions (list[tuple]): One entry per image, each a
                    tuple of (boxes, box_classes, box_scores) as returned
                    by non_max_suppression.
                image_paths (list[str]): Paths to each image file in the
        ...
        # 2. Preprocess: resize + BGR->RGB + normalise
        pimages, image_shapes = self.preprocess_images(images)

        # 3. Batch inference
        # model output is a list of arrays, one per detection scale
        raw_outputs = self.model.predict(pimages)

        predictions = []

        for i, image in enumerate(images):
            # Slice the batch dimension for this single image
            outputs = [scale[i] for scale in raw_outputs]

            # 4. Decode raw predictions into pixel-space boxes
            boxes, box_confs, box_probs = self.process_outputs(
                outputs, image_shapes[i]
            )

            # 5. Filter boxes below the confidence threshold
            filt_boxes, filt_classes, filt_scores = self.filter_boxes(
                boxes, box_confs, box_probs
            )

            # 6. Non-max suppression to remove duplicate detections
            boxes_nms, classes_nms, scores_nms = self.non_max_suppression(
                filt_boxes, filt_classes, filt_scores
            )

            # 7. Annotate and display (optionally save on 's' key)
            self.show_boxes(
                image,
                boxes_nms,
                classes_nms,
            ...
            predictions.append((boxes_nms, classes_nms, scores_nms))
